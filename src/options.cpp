...
error_e options_t::parse(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-k") == 0 || strcmp(argv[i], "--k-bound") == 0) {
            ...
            if (sscanf(argv[++i], "%d", &_query_reduction_timeout) != 1 || _query_reduction_timeout < 0) {
            ...
            if (sscanf(argv[++i], "%d", &_enable_reduction) != 1 || _enable_reduction < 0 || _enable_reduction > 3) {
        ...
        else if (strcmp(argv[i], "-tar") == 0)
        {
            ...
            if (i != argc - 1){
                if (sscanf(argv[++i], "%d", &_max_intervals_reduced) != 1 || _max_intervals_reduced < 0) {
...
//        outputtrace = false;
    ...
    if (_logic == temporal_logic_e::LTL) {
        ...
        std::array ltlStrategies{search_strategy_e::DFS, search_strategy_e::RDFS, search_strategy_e::HEUR};

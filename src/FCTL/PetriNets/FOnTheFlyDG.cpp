...
namespace Featured {
    ...
    namespace PetriNets {
        ...
        FOnTheFlyDG::FOnTheFlyDG(PetriEngine::PetriNet* t_net, bool partial_order)
         : encoder(t_net->numberOfPlaces(), 0),
           edge_alloc(
                   new linked_bucket_t<DependencyGraph::Edge,
                           1024 * 10>(1)),
           conf_alloc(
        ...
        Condition::Result FOnTheFlyDG::initialEval() {
            initialConfiguration();
            EvaluationContext e(query_marking.marking(), net);
            return PetriEngine::PQL::evaluate(query, e);
        }

        Condition::Result FOnTheFlyDG::fastEval(Condition* query, Marking* unfolded) {
            EvaluationContext e(unfolded->marking(), net);
            return PetriEngine::PQL::evaluate(query, e);
        }

        std::vector<DependencyGraph::Edge*> FOnTheFlyDG::successors(Configuration* c) {
        ...
        Configuration* FOnTheFlyDG::initialConfiguration() {
        ...
        void FOnTheFlyDG::setQuery(Condition* query) {
            this->query = query;
        ...
        size_t FOnTheFlyDG::configurationCount() const {
            return _configurationCount;
        }

        size_t FOnTheFlyDG::markingCount() const {
            return _markingCount;
        }

        size_t FOnTheFlyDG::maxTokens() const {
            return _maxTokens;
        }
        ...
        size_t FOnTheFlyDG::createMarking(Marking& t_marking) {
            size_t sum = 0;
            bool allsame = true;
            uint32_t val = 0;
            uint32_t active = 0;
        ...
        void FOnTheFlyDG::release(Edge* e) {
            assert(e->refcnt == 0);
            e->is_negated = false;
            e->processed = false;
            e->source = nullptr;
        ...
        size_t FOnTheFlyDG::owner(Marking& marking, Condition* cond) {
            // Used for distributed algorithm
            return 0;
        }


        Edge* FOnTheFlyDG::newEdge(Configuration& t_source, uint32_t weight) {
            Edge* e = nullptr;
            if (recycle.empty()) {
        ...
        void FOnTheFlyDG::markingStats(const uint32_t* marking, size_t& sum,

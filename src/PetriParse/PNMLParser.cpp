...
void PNMLParser::parse(std::ifstream& xml,
    ...
    for (auto & transition : transitions)
        if (!isColored) {
            builder->add_transition(transition.id, transition.x, transition.y);
        } else {
            builder->add_transition(transition.id, transition.expr, transition.x, transition.y);
        }
    ...
    for (auto & arc : arcs) {
        ...
        if (source.isPlace && !target.isPlace) {
            if (!isColored) {
                builder->add_input_arc(source.id, target.id, arc.inhib, arc.weight);
            } else {
                builder->add_input_arc(source.id, target.id, arc.expr, arc.inhib, arc.weight);
            }

        } else if (!source.isPlace && target.isPlace) {
            if (!isColored) {
                builder->add_output_arc(source.id, target.id, arc.weight);
            } else {
                builder->add_output_arc(source.id, target.id, arc.expr);
            }
        } else {
            fprintf(stderr,
...
void PNMLParser::parsePlace(rapidxml::xml_node<>* element) {
    ...
    //Create place
    if (!isColored) {
        builder->add_place(id, initialMarking, x, y);
    } else {
        if (!type) {
        ...
        {
            builder->add_place(id, type, std::move(hlinitialMarking), x, y);
        }
    }
    //Map id to name
    NodeName nn;

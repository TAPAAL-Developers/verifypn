...
auto main(int argc, char *argv[]) -> int {
    ...
    if (options._cpn_overapprox) {
        for (ssize_t qid = queries.size() - 1; qid >= 0; --qid) {
            ...
            if (!q->is_reachability() || q->is_loop_sensitive() || stats._negated_fireability) {
                std::cerr
                    << "Warning: CPN OverApproximation is only available for Reachability queries "
                       "without deadlock, negated fireability and UpperBounds, skipping "
    ...
    if (options._compute_CFP) {
        cpnBuilder.compute_place_color_fixpoint(
            options._max_intervals, options._max_intervals_reduced, options._interval_timeout);
    }
...

    {
        ...
        if (options._query_out_file.size() > 0) {
            output_queries(b2, queries, querynames, options._query_out_file,
                           options._binary_query_io);
        }
    ...
    if (options._strategy != options_t::search_strategy_e::OVER_APPROX) {
        ...
        if (!ctl_ids.empty()) {
            ...
            for (auto i : ctl_ids) {
                auto r = CTL::verify_ctl(*net, queries[i], options);

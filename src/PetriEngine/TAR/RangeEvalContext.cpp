...
void RangeEvalContext::accept(const CompareConjunction *element) {
    ...
    for (const CompareConjunction::cons_t &c : *element) {
        ...
        if (element->is_negated()) {
            ...
            if (it == nullptr)
                continue; // we are looking for false instance
            if (!(it->_range._lower <= c._upper && c._lower <= it->_range._upper)) {
                _bool_result = false;
                if (it->_range._upper < c._lower) {
                    ...
                    if (!found ||
                        (_use_count[c._place] > _use_count[tmp._place] && !tmp._range.no_upper()))
                    ...
                    if (!found || _use_count[c._place] > _use_count[tmp._place] ||
                        !tmp._range.no_upper())

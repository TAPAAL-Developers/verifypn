...
void Reducer::print(QueryPlaceAnalysisContext &context) {
    std::cout << "\nNET INFO:\n"
              ...
              << "Number of transitions: " << _builder->number_of_transitions() << std::endl
              << std::endl;
    for (uint32_t t = 0; t < _builder->number_of_transitions(); t++) {
        ...
        for (auto &arc : _builder->_transitions[t]._pre) {
            if (arc._weight > 0)
                std::cout << "\tInput place " << arc._place << " (" << get_place_name(arc._place)
                          << ")"
                          << " with arc-weight " << arc._weight << std::endl;
        }
        for (auto &arc : _builder->_transitions[t]._post) {
            if (arc._weight > 0)
                std::cout << "\tOutput place " << arc._place << " (" << get_place_name(arc._place)
                          << ")"
                          << " with arc-weight " << arc._weight << std::endl;
        }
        std::cout << std::endl;
    ...
    for (uint32_t i = 0; i < _builder->number_of_places(); i++) {
        std::cout << "Marking at place " << i << " is: " << _builder->init_marking()[i]
                  << std::endl;
    }
    for (uint32_t i = 0; i < _builder->number_of_places(); i++) {
...
auto Reducer::get_transition(uint32_t transition) -> transition_t & {
    return _builder->_transitions[transition];
}

auto Reducer::get_out_arc(transition_t &trans, uint32_t place) -> ArcIter {
    arc_t a;
    a._place = place;
...
auto Reducer::get_in_arc(uint32_t place, transition_t &trans) -> ArcIter {
    arc_t a;
    a._place = place;
...
void Reducer::skip_transition(uint32_t t) {
    ++_removedTransitions;
    transition_t &trans = get_transition(t);
    assert(!trans._skip);
...
void Reducer::skip_place(uint32_t place) {
    ++_removedPlaces;
    place_t &pl = _builder->_places[place];
    assert(!pl._skip);
    pl._skip = true;
...
auto Reducer::consistent() -> bool {
    ...
    for (size_t i = 0; i < _builder->number_of_transitions(); ++i) {
        transition_t &t = _builder->_transitions[i];
        if (t._skip)
            ++strans;
    ...
    for (size_t i = 0; i < _builder->number_of_places(); ++i) {
        ...
        for (uint c : p._consumers) {
            transition_t &t = _builder->_transitions[c];
            assert(!t._skip);
        ...
        for (uint prod : p._producers) {
            transition_t &t = _builder->_transitions[prod];
            assert(!t._skip);
...
auto Reducer::rule_a(uint32_t *placeInQuery) -> bool {
    ...
    for (uint32_t t = 0; t < number_of_transitions; t++) {
        ...
        if (_builder->_places[pPre]._inhib || trans._inhib)
            continue;

        // A5. dont mess with query!
        if (placeInQuery[pPre] > 0)
            continue;
        // check A1, A4 and A5 for post
        for (auto &pPost : trans._post) {
            if (_builder->_places[pPost._place]._inhib || pPre == pPost._place ||
                placeInQuery[pPost._place] > 0) {
                ok = false;
                break;
            }
        }
        ...
        for (auto &pPost : trans._post) {
            ...
            _builder->_initial_marking[pPost._place] +=
                ((_builder->_initial_marking[pPre] / w) * pPost._weight);
        }
        ...
        for (auto &_t : _builder->_places[pPre]._producers) {
            ...
            transition_t &src = get_transition(_t);
...
auto Reducer::rule_b(uint32_t *placeInQuery, bool remove_deadlocks, bool remove_consumers) -> bool {
    ...
    for (uint32_t p = 0; p < number_of_places; p++) {
        ...
        if (place._consumers.size() != 1 || place._producers.size() < 1)
        ...
        transition_t &in = get_transition(tIn);
        for (auto tOut : prod) {
            transition_t &out = get_transition(tOut);
            ...
            // B6. also, none of the places in the post-set of consuming transition can be
            // participating in inhibitors. B7. nor can they appear in the query.
            {
                bool post_ok = false;
                for (const arc_t &a : in._post) {
...
auto Reducer::rule_c(uint32_t *placeInQuery) -> bool {
    ...
    for (uint32_t touter = 0; touter < _builder->number_of_transitions(); ++touter)
        for (size_t outer = 0; outer < _builder->_transitions[touter]._post.size(); ++outer) {
            ...
            if (has_timed_out())
                return false;
            if (_builder->_places[pouter]._skip)
                continue;

            // C4. No inhib
            if (_builder->_places[pouter]._inhib)
                continue;

            for (size_t inner = outer + 1; inner < _builder->_transitions[touter]._post.size();
                 ++inner) {
                ...
                // C4. No inhib
                if (_builder->_places[pinner]._inhib)
                    continue;

                for (size_t swp = 0; swp < 2; ++swp) {
                    if (has_timed_out())
                        return false;
                    if (_builder->_places[pinner]._skip || _builder->_places[pouter]._skip)
                        break;
                    ...
                    for (unsigned int _consumer : place2._consumers) {
                        ...
                        transition_t &trans = get_transition(place1._consumers[j]);
                        auto a1 = get_in_arc(p1, trans);
                        auto a2 = get_in_arc(p2, trans);
                        assert(a1 != trans._pre.end());
                        assert(a2 != trans._pre.end());
                        mult =
                            std::max(mult, ((long double)a2->_weight) / ((long double)a1->_weight));
                    }
                    ...
                    if (mult != std::numeric_limits<long double>::max() &&
                        (((long double)_builder->_initial_marking[p1]) * mult) >
                            ((long double)_builder->_initial_marking[p2])) {
                        continue;
                    }
                    ...
                    for (unsigned int _producer : place1._producers) {
                        ...
                        transition_t &trans = get_transition(_producer);
    ...
    for (auto &op : _builder->_places)
        for (size_t outer = 0; outer < op._consumers.size(); ++outer) {
            ...
            if (_tflags[touter] != 0)
                continue;
            _tflags[touter] = 1;
            transition_t &tout = get_transition(touter);
            if (tout._skip)
                continue;
            ...
            for (size_t inner = outer + 1; inner < op._consumers.size(); ++inner) {
                ...
                transition_t &tin = get_transition(tinner);
                if (tin._skip || tout._skip)
                    continue;
                ...
                for (size_t swp = 0; swp < 2; ++swp) {
                    ...
                    transition_t &trans1 = get_transition(t1);
                    transition_t &trans2 = get_transition(t2);

                    // From D3, and D4 we have that pre and post-sets are the same
                    if (trans1._post.size() != trans2._post.size())
                        break;
                    if (trans1._pre.size() != trans2._pre.size())
                        break;
                    ...
                    for (int i = (int)trans1._post.size() - 1; i >= 0; --i) {
                        arc_t &arc = trans1._post[i];
                        arc_t &arc2 = trans2._post[i];
                    ...
                    for (int i = (int)trans1._pre.size() - 1; i >= 0; --i) {
                        arc_t &arc = trans1._pre[i];
                        arc_t &arc2 = trans2._pre[i];
                        if (arc2._place != arc._place) {
                            ok = 2;
                            break;
                        }
...
auto Reducer::rule_e(uint32_t *placeInQuery) -> bool {
    ...
    for (uint32_t p = 0; p < number_of_places; ++p) {
        ...
        if (place._inhib)
            continue;
        if (place._producers.size() > place._consumers.size())
            continue;

        std::set<uint32_t> notenabled;
        bool ok = true;
        for (uint cons : place._consumers) {
            transition_t &t = get_transition(cons);
        ...
        for (uint prod : place._producers) {
            ...
            transition_t &t = get_transition(prod);
    ...
    for (uint32_t p = 0; p < number_of_places; ++p) {
        ...
        for (uint cons : place._consumers) {
            ...
            if (w > _builder->_initial_marking[p]) {
                ...
                if (it == t._post.end() || it->_place != p || it->_weight < w) {
                    ok = false;
                    break;
                }
            }
...
auto Reducer::rule_g(uint32_t *placeInQuery, bool remove_loops, bool remove_consumers) -> bool {
    ...
    for (uint32_t t = 0; t < _builder->number_of_transitions(); ++t) {
        if (has_timed_out())
            return false;
        transition_t &trans = _builder->_transitions[t];
        if (trans._skip)
            continue;
        if (trans._inhib)
            continue;
        if (trans._pre.size() < trans._post.size())
            continue;
        if (!remove_loops && trans._pre.size() == 0)
            continue;
...
auto Reducer::rule_h(uint32_t *placeInQuery) -> bool {
    ...
    auto transok = [this](uint32_t t) -> uint32_t {
        ...
        if (_tflags[t] != 0)
            return _tflags[t];
        _tflags[t] = 1;
        if (trans._inhib || trans._pre.size() != 1 || trans._post.size() != 1) {
            return 2;
        }
        ...
        if (trans._pre[0]._weight != 1 || trans._post[0]._weight != 1 || p1 == p2 ||
            _builder->_places[p1]._inhib || _builder->_places[p2]._inhib) {
            return 2;
        }
        return 1;
    };
...

            {
                ...
                for (auto p2it : place2._consumers) {
                    ...
                    if (dest == t._pre.end() || dest->_place != p1) {
                        ...
                        auto lb = std::lower_bound(place1._consumers.begin(),
                                                   place1._consumers.end(), p2it);
...
auto Reducer::rule_j(uint32_t *placeInQuery) -> bool {
    ...
    for (uint32_t t = 0; t < _builder->number_of_transitions(); ++t) {
        ...
        if (_builder->_transitions[t]._skip || _builder->_transitions[t]._inhib ||
        ...
        place_t pp = place;
        skip_place(p);
        std::vector<std::vector<arc_t>> posts;
        std::vector<transition_t> pres;
...
auto Reducer::relevant(const uint32_t *placeInQuery, bool remove_consumers)
    ...
    while (!wtrans.empty()) {
        ...
        auto t = wtrans.back();
        wtrans.pop_back();
        const transition_t &trans = _builder->_transitions[t];
        for (const arc_t &arc : trans._pre) {
            const place_t &place = _builder->_places[arc._place];
            if (arc._inhib) {
                for (auto pt : place._consumers) {
                    if (!tseen[pt]) {
                        transition_t &trans = _builder->_transitions[pt];
                        auto it = trans._post.begin();
                        for (; it != trans._post.end(); ++it)
                            if (it->_place >= arc._place)
                                break;
                        if (it != trans._post.end() && it->_place == arc._place) {
                            auto it2 = trans._pre.begin();
                            for (; it2 != trans._pre.end(); ++it2)
                                if (it2->_place >= arc._place || it2->_inhib)
                                    break;
                            if (it->_weight <= it2->_weight)
                                continue;
                        }
                        tseen[pt] = true;
                        wtrans.push_back(pt);
...
std::array tnames{"T-lb_balancing_receive_notification_10",
...
void Reducer::reduce(QueryPlaceAnalysisContext &context, int enablereduction, bool reconstructTrace,
    ...
    if (enablereduction == 2) { // for k-boundedness checking only rules A, D and H are applicable
    ...
    } else if (enablereduction == 1) { // in the aggressive reduction all four rules are used as
                                       // long as they remove something
        bool changed = false;
        do {
            ...
            do {
                do { // start by rules that do not move tokens
                    ...
                    if (!next_safe) {
                        ...
                        // that it is not correct for CTL.
                    }
                } while (changed && !has_timed_out());
        ...
        for (int i = (int)reduction.size() - 1; i >= 0; --i) {
            if (next_safe) {
                if (reduction[i] != 2 && reduction[i] != 4 && reduction[i] != 5) {
                    std::cerr << "Skipping Rule" << rnames[reduction[i]]
                              << " due to NEXT operator in proposition" << std::endl;
            ...
            if (!remove_loops && reduction[i] == 5) {
                std::cerr << "Skipping Rule" << rnames[reduction[i]]
                          << " as proposition is loop sensitive" << std::endl;
                reduction.erase(reduction.begin() + i);
            }
        }
        bool changed = true;

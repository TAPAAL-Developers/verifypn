...
namespace PetriEngine {
    namespace Colored {
        ...
        bool PartitionBuilder::partition_net(int32_t timeout) {
            ...
            while(!_placeQueue.empty() && timeout > 0 && std::chrono::duration_cast<std::chrono::seconds>(end - start).count() < timeout){
                ...
                if(allPositionsDiagonal || _partition[placeId].get_eq_classes().size() >=
                    _partition[placeId].get_eq_classes().back().type()->size(_partition[placeId].get_dagonal_tuple_positions())){
        ...
        void PartitionBuilder::assign_color_map(std::unordered_map<uint32_t, EquivalenceVec> &partition) const{
                ...
                for(const auto &variable : preVarPositionMap){
                    for(const auto &varPosition : placeVariables.second){
                        if(varPosition.second == variable.second){
                            ...
                            if(_partition[placeVariables.first].get_eq_classes().back().type()->product_size() == 1){
                                _partition[placeVariables.first].set_diagonal(true);
                                add_to_queue(placeVariables.first);
                            } else if(!_partition[placeVariables.first].get_dagonal_tuple_positions()[varPosition.first]) {
                                add_to_queue(placeVariables.first);
                                _partition[placeVariables.first].set_diagonal_tuple_position(varPosition.first, true);
                            }
                            break;
                        }
                    }
            ...
            for(const auto &preVar : preVarPositionMap){
                for(const auto &postVar : varPositionMap){
                    if(preVar.second == postVar.second){
                        if(_partition[postPlaceId].is_diagonal() || _partition[postPlaceId].get_dagonal_tuple_positions()[postVar.first]){
                            if(_partition[prePlaceId].get_eq_classes().back().type()->product_size() == 1){
                                _partition[prePlaceId].set_diagonal(true);
                            } else if(!_partition[prePlaceId].get_dagonal_tuple_positions()[preVar.first]) {
        ...
        void PartitionBuilder::apply_new_intervals(const Arc &inArc, const std::vector<PetriEngine::Colored::VariableIntervalMap> &varMaps){
            ...
            for(auto& intervalTuple : outIntervals){
                ...
                EquivalenceClass newEqClass(++_eq_id_counter, _partition[inArc._place].get_eq_classes().back().type(), std::move(intervalTuple));
                newEqVec.push_back_eq_class(std::move(newEqClass));
            }
        ...
        void PartitionBuilder::handle_transition(const Transition &transition, const uint32_t postPlaceId, const Arc *postArc) {
            ...
            const std::vector<Colored::EquivalenceClass> placePartition = _partition[postPlaceId].get_eq_classes();
        ...
        void PartitionBuilder::handle_in_arcs(const Transition &transition, std::set<const Colored::Variable*> &diagonalVars, const PositionVariableMap &varPositionMap, const std::vector<PetriEngine::Colored::VariableIntervalMap> &varMaps, uint32_t postPlaceId){
            ...
            for(const auto &inArc : transition._input_arcs){
                ...
                if(check_diagonal(inArc._place)) continue;
                ...
                if(_partition[inArc._place].get_eq_classes().size() >=
                    _partition[inArc._place].get_eq_classes().back().type()->size(_partition[inArc._place].get_dagonal_tuple_positions())){
                    _partition[inArc._place].set_diagonal(true);
                    continue;
                }
        ...
        bool PartitionBuilder::split_partition(PetriEngine::Colored::EquivalenceVec equivalenceVec, uint32_t placeId){
            ...
            if(_partition.count(placeId) == 0){
                ...
                while(find_overlap(equivalenceVec, _partition[placeId],ecPos1, ecPos2, intersection)) {
                    ...
                    const auto rightSubtractEc = ec1.subtract(++_eq_id_counter, ec2, equivalenceVec.get_dagonal_tuple_positions());
                    const auto leftSubtractEc = ec2.subtract(++_eq_id_counter, ec1, _partition[placeId].get_dagonal_tuple_positions());
        ...
        void PartitionBuilder::handle_leaf_transitions(){
            for(uint32_t i = 0; i < _transitions.size(); i++){
                ...
                if(transition._output_arcs.empty() && !transition._input_arcs.empty()){
                    handle_transition(transition, transition._input_arcs.back()._place, &transition._input_arcs.back());
                }
            }
        }
    }

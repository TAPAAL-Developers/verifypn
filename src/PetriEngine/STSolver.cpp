...
STSolver::STSolver(const Reachability::ResultPrinter &printer, const PetriNet &net,
...
auto STSolver::_compute_trap(std::vector<size_t> &trap, const std::set<size_t> &preset,
    ...
    if (trap.empty())
        return 0;
    // compute DIFF = T* \ *T
    auto eit = std::set_difference(postset.begin(), postset.end(), preset.begin(), preset.end(),
                                   _diff.begin());
    if (eit == _diff.begin()) {
        // DIFF = empty
        if (marked_count > 0) {
        ...
        for (auto it = _diff.begin(); it != eit; ++it) {
            ...
            auto sit = trap.begin();
            for (; pre.first != pre.second; ++pre.first) {
                while (sit != std::end(trap) && *sit < pre.first->_place)
                    ++sit;
                if (sit == std::end(trap))
                    break;
...
void STSolver::extend(size_t place, std::set<size_t> &pre, std::set<size_t> &post) {
    ...
    post.insert(_transitions.get() + _places[place]._post,
...
auto STSolver::siphon_trap(std::vector<size_t> siphon, const std::vector<bool> &has_st,
    ...
    auto eit = std::set_difference(preset.begin(), preset.end(), postset.begin(), postset.end(),
                                   _diff.begin());
    if (eit == _diff.begin()) {
        ...
        for (; pre.first != pre.second; ++pre.first) {
            while (sit != siphon.end() && *sit < pre.first->_place)
                ++sit;
            if (sit != siphon.end() && *sit == pre.first->_place)
                continue;

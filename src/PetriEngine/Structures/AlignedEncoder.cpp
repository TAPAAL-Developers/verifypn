/*
 * File:   Encoder.cpp
 * Author: Peter G. Jensen
 *
 * Created on 11 March 2016, 14:15
 */

#include <limits>

#include "PetriEngine/Structures/AlignedEncoder.h"

#define SAMEBOUND 120
#define DBOUND (SAMEBOUND * 2)

AlignedEncoder::AlignedEncoder(uint32_t places) : _places(places) {

    size_t bytes = 2 * sizeof(uint32_t) + (places * sizeof(uint32_t));
    _scratchpad = scratchpad_t(bytes * 8);
    assert(_scratchpad.size() == (2 * sizeof(uint32_t) + (_places * sizeof(uint32_t))));
    if (_places < 256)
        _psize = 1;
    else if (_places < 65536)
        _psize = 2;
    else
        _psize = 4;

    assert(_psize != 0);
}

AlignedEncoder::~AlignedEncoder() { _scratchpad.release(); }

auto AlignedEncoder::token_bytes(uint32_t ntokens) const -> uint32_t {
    uint32_t size;
    if (ntokens < 256)
        size = 1;
    else if (ntokens < 65536)
        size = 2;
    else
        size = 4;
    return size;
}

auto AlignedEncoder::write_bit_vector(const uint32_t *data) -> uint32_t {
    for (size_t i = 0; i < _places; ++i) {
        _scratchpad.set(i + (1 * 8), data[i] > 0);
    }
    return 1 + scratchpad_t::bytes(_places);
}

auto AlignedEncoder::write_two_bit_vector(const uint32_t *data) -> uint32_t {
    for (size_t i = 0; i < _places; ++i) {
        switch (data[i]) {
        case 1:
            _scratchpad.set((i * 2) + (1 * 8), true);
            break;
        case 3:
            _scratchpad.set((i * 2) + (1 * 8), true);
        case 2:
            _scratchpad.set((i * 2) + (1 * 8) + 1, true);
            break;
        default:
            break;
        }
    }

    return 1 + scratchpad_t::bytes(_places * 2);
}

auto AlignedEncoder::read_two_bit_vector(uint32_t *destination, const unsigned char *source)
    -> uint32_t {
    scratchpad_t b = scratchpad_t((unsigned char *)&source[1], _places * 2);
    for (size_t i = 0; i < _places; ++i) {
        destination[i] = 0;
        if (b.at((i * 2))) {
            destination[i] = 1;
        }

        if (b.at((i * 2) + 1)) {
            destination[i] += 2;
        }
    }
    return 1 + scratchpad_t::bytes(_places * 2);
}

template <typename T> auto AlignedEncoder::write_tokens(const uint32_t *data) -> uint32_t {
    if (sizeof(T) == sizeof(uint32_t)) {
        memcpy(&(_scratchpad.raw()[1]), data, _places * sizeof(T));
    } else {
        for (size_t i = 0; i < _places; ++i) {
            T *dest = (T *)(&_scratchpad.raw()[1 + (i * sizeof(T))]);
            *dest = data[i];
        }
    }
    return 1 + _places * sizeof(T);
}

template <typename T>
auto AlignedEncoder::read_tokens(uint32_t *destination, const unsigned char *source) -> uint32_t {
    for (size_t i = 0; i < _places; ++i) {
        T *src = (T *)(&source[1 + (i * sizeof(T))]);
        destination[i] = *src;
    }
    return 1 + _places * sizeof(T);
}

template <typename T>
auto AlignedEncoder::write_token_counts(size_t offset, const uint32_t *data) -> uint32_t {
    size_t cnt = 0;

    for (size_t i = 0; i < _places; ++i) {
        if (data[i] > 0) {
            T *dest = (T *)(&_scratchpad.raw()[offset + (cnt * sizeof(T))]);
            *dest = data[i];
            ++cnt;
        }
    }
    return offset + cnt * sizeof(T);
}

template <typename T>
auto AlignedEncoder::bit_token_counts_size(const unsigned char *source) const -> size_t {
    scratchpad_t b = scratchpad_t((unsigned char *)&source[1], _places);

    size_t cnt = 0;
    for (uint32_t i = 0; i < _places; ++i) {
        if (b.at(i)) {
            cnt += sizeof(T);
        }
    }
    return 1 + b.size() + cnt;
}

template <typename T>
auto AlignedEncoder::read_bit_token_counts(uint32_t *destination, const unsigned char *source) const
    -> uint32_t {
    const unsigned char *ts = &source[1 + scratchpad_t::bytes(_places)];
    scratchpad_t b = scratchpad_t((unsigned char *)&source[1], _places);

    size_t cnt = 0;
    for (uint32_t i = 0; i < _places; ++i) {
        if (b.at(i)) {
            destination[i] = *((T *)&ts[cnt]);
            cnt += sizeof(T);
        }
    }
    return 0;
}

template <typename T>
auto AlignedEncoder::place_token_counts_size(const unsigned char *source, uint32_t offset) const
    -> size_t {
    size_t size;
    switch (_psize) {
    case 1:
        size = source[offset];
        break;
    case 2:
        size = *(uint16_t *)(&source[offset]);
        break;
    case 4:
        size = *(uint32_t *)(&source[offset]);
        break;
    default:
        size = std::numeric_limits<size_t>::max(); // should provoke an error
        assert(false);
    }
    offset += _psize;
    return offset + (_psize * size) + (size * sizeof(T));
}

template <typename T>
auto AlignedEncoder::read_place_token_counts(uint32_t *destination, const unsigned char *source,
                                             uint32_t offset) const -> uint32_t {
    size_t size;
    switch (_psize) {
    case 1:
        size = source[offset];
        break;
    case 2:
        size = *(uint16_t *)(&source[offset]);
        break;
    case 4:
        size = *(uint32_t *)(&source[offset]);
        break;
    default:
        size = std::numeric_limits<size_t>::max(); // should provoke an error
        assert(false);
    }

    offset += _psize;
    const unsigned char *ts = &source[offset + (_psize * size)];
    for (size_t i = 0; i < size; ++i) {
        size_t pos = 0;
        switch (_psize) {
        case 1:
            pos = source[offset + i];
            break;
        case 2:
            pos = *((uint16_t *)&source[offset + i * 2]);
            break;
        case 4:
            pos = *((uint32_t *)&source[offset + i * 4]);
            break;
        default:
            assert(false);
            break;
        }

        destination[pos] = *((T *)&ts[i * sizeof(T)]);
    }
    return offset + size;
}

auto AlignedEncoder::write_places(const uint32_t *data) -> uint32_t {
    size_t cnt = 0;
    uint16_t *dest16 = (uint16_t *)(&_scratchpad.raw()[1]);
    uint32_t *dest32 = (uint32_t *)(&_scratchpad.raw()[1]);
    for (size_t i = 0; i < _places; ++i) {
        if (data[i] > 0) {
            switch (_psize) {
            case 1:
                _scratchpad.raw()[1 + cnt + 1] = (unsigned char)i;
                break;
            case 2:
                dest16[cnt + 1] = (uint16_t)i;
                break;
            case 4:
                dest32[cnt + 1] = i;
                break;
            default:
                assert(false);
            }
            ++cnt;
        }
    }

    switch (_psize) {
    case 1:
        _scratchpad.raw()[1] = (unsigned char)cnt;
        break;
    case 2:
        dest16[0] = cnt;
        break;
    case 4:
        dest32[0] = cnt;
        break;
    default:
        assert(false);
    }
    return 1 + _psize + cnt * _psize;
}

auto AlignedEncoder::read_places(uint32_t *destination, const unsigned char *source,
                                 uint32_t offset, uint32_t value) -> uint32_t {
    size_t size;
    switch (_psize) {
    case 1:
        size = source[offset];
        break;
    case 2:
        size = *(uint16_t *)(&source[offset]);
        break;
    case 4:
        size = *(uint32_t *)(&source[offset]);
        break;
    default:
        size = std::numeric_limits<size_t>::max(); // should provoke an error
        assert(false);
    }

    offset += _psize;

    uint16_t *raw16 = (uint16_t *)&source[offset];
    uint32_t *raw32 = (uint32_t *)&source[offset];
    for (size_t i = 0; i < size; ++i) {
        switch (_psize) {
        case 1:
            destination[source[i + offset]] = value;
            break;
        case 2:
            destination[raw16[i]] = value;
            break;
        case 4:
            destination[raw32[i]] = value;
            break;
        default:
            assert(false);
        }
    }
    return offset + _psize * size;
}

auto AlignedEncoder::read_bit_vector(uint32_t *destination, const unsigned char *source,
                                     uint32_t value) -> uint32_t {
    scratchpad_t b = scratchpad_t((unsigned char *)&source[1], _places);
    for (uint32_t i = 0; i < _places; ++i) {
        if (b.at(i)) {
            destination[i] = value;
        } else {
            destination[i] = 0;
        }
    }
    return 1 + b.size();
}

auto AlignedEncoder::get_type(uint32_t sum, uint32_t pwt, bool same, uint32_t val) const
    -> unsigned char {
    if (pwt == 0)
        return 0;
    if (same && val <= SAMEBOUND) {
        size_t bvsize = scratchpad_t::bytes(_places);
        size_t indirect = _psize + pwt * _psize;

        if (bvsize <= indirect) {
            return val;
        } else {
            return SAMEBOUND + val;
        }
    } else {
        size_t tsize = token_bytes(val);
        size_t bvsize = scratchpad_t::bytes(_places * 2);
        size_t indirect = _psize + pwt * (_psize + tsize);
        size_t bvindirect = scratchpad_t::bytes(_places) + pwt * tsize;
        size_t direct = _places * tsize;

        if (val < 4 && bvsize <= indirect && bvsize <= bvindirect) {
            return DBOUND + 1;
        } else if (direct <= indirect && direct <= bvindirect) {
            switch (tsize) {
            case 1:
                return DBOUND + 2;
            case 2:
                return DBOUND + 3;
            case 4:
                return DBOUND + 4;
            default:
                assert(false);
            }
        } else if (indirect <= bvindirect) {
            switch (tsize) {
            case 1:
                return DBOUND + 5;
            case 2:
                return DBOUND + 6;
            case 4:
                return DBOUND + 7;
            default:
                assert(false);
            }
        } else {
            switch (tsize) {
            case 1:
                return DBOUND + 8;
            case 2:
                return DBOUND + 9;
            case 4:
                return DBOUND + 10;
            default:
                assert(false);
            }
        }
    }
    assert(false);
    return 0;
}

auto AlignedEncoder::size(const uchar *s) const -> size_t {
    unsigned char type = s[0];
    if (type <= SAMEBOUND) {
        if ((_places % 8) == 0)
            return 1 + (_places / 8);
        else
            return 2 + (_places / 8);
    }

    if (type <= DBOUND) {
        size_t size;
        switch (_psize) {
        case 1:
            size = 1 + s[1];
            break;
        case 2:
            size = 1 + *(uint16_t *)(&s[1]);
            size *= sizeof(uint16_t);
            break;
        case 4:
            size = 1 + *(uint32_t *)(&s[1]);
            size *= sizeof(uint32_t);
            break;
        default:
            size = std::numeric_limits<size_t>::max(); // should provoke an error
            assert(false);
        }
        return size + 1;
    }

    switch (type) {
    case DBOUND + 1:
        if ((_places % 4) == 0)
            return 1 + (_places / 4);
        else
            return 2 + (_places / 4);
    case DBOUND + 2:
        return 1 + (sizeof(unsigned char) * _places);
    case DBOUND + 3:
        return 1 + (sizeof(uint16_t) * _places);
    case DBOUND + 4:
        return 1 + (sizeof(uint32_t) * _places);
    case DBOUND + 5:
        return place_token_counts_size<unsigned char>((unsigned char *)s, 1);
    case DBOUND + 6:
        return place_token_counts_size<uint16_t>((unsigned char *)s, 1);
    case DBOUND + 7:
        return place_token_counts_size<uint32_t>((unsigned char *)s, 1);
    case DBOUND + 8:
        return bit_token_counts_size<unsigned char>((unsigned char *)s);
    case DBOUND + 9:
        return bit_token_counts_size<uint16_t>((unsigned char *)s);
    case DBOUND + 10:
        return bit_token_counts_size<uint32_t>((unsigned char *)s);
    default:
        assert(false);
        return std::numeric_limits<size_t>::infinity();
    }
}

auto AlignedEncoder::encode(const uint32_t *d, unsigned char type) -> size_t {
    _scratchpad.zero();
    _scratchpad.raw()[0] = type;
    if (type <= SAMEBOUND) {
        return write_bit_vector(d);
    }
    if (type <= DBOUND) {
        return write_places(d);
    }

    switch (type) {
    case DBOUND + 1:
        return write_two_bit_vector(d);
    case DBOUND + 2:
        return write_tokens<unsigned char>(d);
    case DBOUND + 3:
        return write_tokens<uint16_t>(d);
    case DBOUND + 4:
        return write_tokens<uint32_t>(d);
    case DBOUND + 5: {
        size_t size = write_places(d);
        return write_token_counts<unsigned char>(size, d);
    }
    case DBOUND + 6: {
        size_t size = write_places(d);
        return write_token_counts<uint16_t>(size, d);
    }
    case DBOUND + 7: {
        size_t size = write_places(d);
        return write_token_counts<uint32_t>(size, d);
    }
    case DBOUND + 8: {
        size_t size = write_bit_vector(d);
        return write_token_counts<unsigned char>(size, d);
    }
    case DBOUND + 9: {
        size_t size = write_bit_vector(d);
        return write_token_counts<uint16_t>(size, d);
    }
    case DBOUND + 10: {
        size_t size = write_bit_vector(d);
        return write_token_counts<uint32_t>(size, d);
    }
    default:
        assert(false);
    }
    assert(false);
    return 0;
}

void AlignedEncoder::decode(uint32_t *d, const unsigned char *s) {
    memset(d, 0, sizeof(uint32_t) * _places);
    unsigned char type = s[0];
    if (type <= SAMEBOUND) {
        read_bit_vector(d, s, type);
        return;
    }
    if (type <= DBOUND) {
        read_places(d, s, 1, type - SAMEBOUND);
        return;
    }

    switch (type) {
    case DBOUND + 1:
        read_two_bit_vector(d, s);
        return;
    case DBOUND + 2:
        read_tokens<unsigned char>(d, s);
        return;
    case DBOUND + 3:
        read_tokens<uint16_t>(d, s);
        return;
    case DBOUND + 4:
        read_tokens<uint32_t>(d, s);
        return;
    case DBOUND + 5:
        read_place_token_counts<unsigned char>(d, s, 1);
        return;
    case DBOUND + 6:
        read_place_token_counts<uint16_t>(d, s, 1);
        return;
    case DBOUND + 7:
        read_place_token_counts<uint32_t>(d, s, 1);
        return;
    case DBOUND + 8:
        read_bit_token_counts<unsigned char>(d, s);
        return;
    case DBOUND + 9:
        read_bit_token_counts<uint16_t>(d, s);
        return;
    case DBOUND + 10:
        read_bit_token_counts<uint32_t>(d, s);
        return;
    default:
        assert(false);
    }
}

...
// Causes FORMULA <qname> STATS EXPLORED <nexplored> to be printed after verification. Remove if
 ...
 * @return @code(ltl_formula, should_negate) - ltl_formula is the formula f if it is a valid LTL
...
std::unique_ptr<Heuristic> make_heuristic(const PetriNet &net, const Condition_ptr &negated_formula,
    ...
    if (options._strategy != options_t::search_strategy_e::HEUR &&
        options._strategy != options_t::search_strategy_e::DEFAULT) {
        return nullptr;
    }
...
bool verify_ltl(const PetriNet &net, const Condition_ptr &query, const std::string &queryName,
    ...
    case Algorithm::Tarjan:
        if (options._strategy != options_t::search_strategy_e::DFS || is_stubborn) {
            ...
            // generator to enable heuristics.
            SpoolingSuccessorGenerator gen{net, negated_formula};
            if (is_visible_stub) {

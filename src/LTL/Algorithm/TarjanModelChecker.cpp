    ...
    for (auto &state : initial_states) {
        ...
        while (!_dstack.empty() && !_violation) {
            ...
            if (fired >= std::numeric_limits<uint32_t>::max() - 3) {
                ...
                // state.print(*this->net, std::cerr); std::cerr << ": <transition id='" <<
                // this->net->transitionNames()[fired] << "'>" << std::endl ;
            }
#endif
            ...
            while (suc_pos != std::numeric_limits<idx_t>::max() &&
                   _cstack[suc_pos]._stateid != stateid) {
                if constexpr (_is_spooling) {
                    if (_cstack[suc_pos]._dstack &&
                        _seen.get_marking_id(_cstack[suc_pos]._stateid) == marking) {
...
void TarjanModelChecker<S, G, SaveTrace, Spooler...>::update(idx_t to) {
    ...
    assert(_cstack[to]._lowlink != std::numeric_limits<idx_t>::max() &&
           _cstack[from]._lowlink != std::numeric_limits<idx_t>::max());
...
template <template <typename, typename...> typename S, typename G, bool SaveTrace,
          typename... Spooler>
auto TarjanModelChecker<S, G, SaveTrace, Spooler...>::next_trans(
    State &state, State &parent, TarjanModelChecker::d_entry_t &delem) -> bool {
...
template class TarjanModelChecker<ProductSuccessorGenerator, LTL::ResumingSuccessorGenerator, true>;

template class TarjanModelChecker<ProductSuccessorGenerator, LTL::ResumingSuccessorGenerator,
                                  false>;

template class TarjanModelChecker<ProductSuccessorGenerator, LTL::SpoolingSuccessorGenerator, true>;

template class TarjanModelChecker<ProductSuccessorGenerator, LTL::SpoolingSuccessorGenerator,
                                  false>;

template class TarjanModelChecker<ReachStubProductSuccessorGenerator,
                                  LTL::SpoolingSuccessorGenerator, true, VisibleLTLStubbornSet>;

template class TarjanModelChecker<ReachStubProductSuccessorGenerator,
                                  LTL::SpoolingSuccessorGenerator, false, VisibleLTLStubbornSet>;

template class TarjanModelChecker<ReachStubProductSuccessorGenerator,
                                  LTL::SpoolingSuccessorGenerator, true, EnabledSpooler>;

template class TarjanModelChecker<ReachStubProductSuccessorGenerator,
                                  LTL::SpoolingSuccessorGenerator, false, EnabledSpooler>;
} // namespace LTL

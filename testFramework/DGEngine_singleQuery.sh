...
OUTPUT="$mname-$qname-$aname-$sname-$5.log"
ENGINE="-ctl"
TIMEOUT=900
...
case $aname in
	"local" )
                echo "Algorithm: $aname" >> $mname-$qname-$aname-$sname-$5.log;
	...
	"local-i" )
                ...
                echo "Search: $sname" >> "$OUTPUT"
                echo "Query type: $qname"  >> "$OUTPUT"
                { timeout $TIMEOUT "$1" "$2" "$3" "$4" "local-i" "-s" "$sname" "-x" "$5"; } >> "$OUTPUT" 2>&1
                ;;
	"global" )
                echo "Algorithm: $aname" >> $mname-$qname-$aname-$sname-$5.log;
		echo "Search: $sname" >> "$OUTPUT"
	...
	"global-i" )
                ...
                echo "Search: $sname" >> "$OUTPUT"
                echo "Query type: $qname"  >> "$OUTPUT"
                { timeout $TIMEOUT "$1" "$2" "$3" "$4" "global-i" "-s" "$sname" "-x" "$5"; } >> "$OUTPUT" 2>&1
                ;;
	"czero" )
	...
	"czero-i" )
                ...
                echo "Search: $sname" >> "$OUTPUT"
                echo "Query type: $qname"  >> "$OUTPUT"
                { timeout $TIMEOUT "$1" "$2" "$3" "$4" "czero-i" "-s" "$sname" "-x" "$5"; }  >> "$OUTPUT" 2>&1
                ;;
esac

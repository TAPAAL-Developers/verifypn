...
// Path to save the autogenerated and temporary file
//const char sourcename[] = "/home/isabella/Documents/verifypnLTSmin/autogenerated/autogeneratedfile.c";
//const char sourcename_temp[] = "/home/isabella/Documents/verifypnLTSmin/temp.txt";

const char sourcename[] = "/Users/dyhr/Bazaar/verifypnLTSmin/autogenerated/autogeneratedfile.c";
const char sourcename_temp[] = "/Users/dyhr/Bazaar/verifypnLTSmin/temp.txt";

using namespace std;
namespace PetriEngine{

    CodeGenerator::CodeGenerator(PetriNet* net, MarkVal* m0, PNMLParser::InhibitorArcList inhibarcs, string statelabel, bool isReachBound, bool isPlaceBound, bool quickSolve, QueryXMLParser *Parser) {
        _net = net;
    ...
    void CodeGenerator::generateSource(bool *searchAllPaths, int query_id){
        ...
        successor_generator = fopen(sourcename_temp, "w+");
        ...
        if(_isPlaceBound || _isReachBound) fputs("void __attribute__ ((destructor)) cleanup(void);\n", successor_generator);
     ...
     if (_isPlaceBound){
                    fprintf(successor_generator, "static int MAX[%d] = {", query_id);
       ...
       if (_quickSolve){
        ...
        if (_isReachBound){
            fprintf(successor_generator, "int placebound%d[] = {", query_id);
            for(int i = 0; i < _parser->queries[query_id].numberOfPlaces.size(); i++){
                if(i > 0)
                    fprintf(successor_generator, ", 0");
                else
                    fprintf(successor_generator, "0");
            ...
            fprintf(successor_generator, "char *placeboundExpr[] = {");
            for(int i = 0; i < _parser->queries[query_id].placebounds.size(); i++){
                if(i > 0)
                    fprintf(successor_generator, ", \"%s\"", _parser->queries[query_id].placebounds[i].c_str());
                else
                    fprintf(successor_generator, "\"%s\"", _parser->queries[query_id].placebounds[i].c_str());
            }
            fprintf(successor_generator, "};\n");            
        ...
        for (t = 0; t < _ntransitions; t++){
            ...
            for(p = 0; p< _nplaces; p++){
                if(_net->outArc(t,p) > 0){
                    fprintf(successor_generator, "placemarkings[%d] = placemarkings[%d] + %d;\n", p, p, _net->outArc(t,p));
                    fprintf(successor_generator, "cpy[%d] = 0;\n", p);
                    if(_isReachBound) {fprintf(successor_generator, "if(placemarkings[%d] >= %s[%d]) { %s[%d] = placemarkings[%d]; }\n", p, ComputeBoundsArray, p, ComputeBoundsArray, p, p);}
                }
            }
        ...
        // ReachabilityBounds without short circuit
            ...
            for (int i = 0; i < _parser->queries[query_id].placebounds.size(); ++i)
            {
                fprintf(successor_generator, "if(%s > placebound%d[%d]){ placebound%d[%d] = %s; }\n",  _parser->queries[query_id].placebounds[i].c_str(), query_id, i, query_id, i, _parser->queries[query_id].placebounds[i].c_str());
            }
             fprintf(successor_generator, "return label == LABEL_GOAL && 0;\n}\n");
        }
        ...
        if (_isReachBound && !_quickSolve){
            ...
            fprintf(successor_generator, "if(%s){fprintf(stderr, \"#Query %d is satisfied.\\n\");} else fprintf(stderr, \"#Query %d unable to decide.\\n\"); \n", sl(), query_id, query_id);
            fprintf(successor_generator, "}"); // end cleanup
        ...
        // PlaceBounds
            ...
            fprintf(successor_generator, "fprintf(stderr, \"Query %d max tokens are \'", query_id);
             fputs("%d\'.\\n\", MAX[", successor_generator);
            fprintf(successor_generator, "%d]); \n", query_id);
            fputs("}", successor_generator);  // end cleanup
        ...
        fprintf(successor_generator, "static int MAX[%d] = {", numberOfQueries);
        for(q = 0; q<numberOfQueries; q++){
            if(q == numberOfQueries-1) // dont print a comma after the last query
                fprintf(successor_generator, "0};\n");
            else
        ...
        if (_isReachBound){
        for(q = 0; q<numberOfQueries; q++){
            fprintf(successor_generator, "int placebound%d[] = {", q);
            for(int i = 0; i < _parser->queries[q].numberOfPlaces.size(); i++){
                if(i > 0)
                    fprintf(successor_generator, ", 0");
                else
                    fprintf(successor_generator, "0");
        ...
        fprintf(successor_generator, "int group_count() {return %d;}\nint state_length() {return %d;}\nint label_count() {return %d;}\n", group_count, state_count, label_count);
        fputs("int i;", successor_generator);;
        ...
        if (_isPlaceBound){
            for(q = 0; q<numberOfQueries; q++){
                    fprintf(successor_generator, "%s", statelabels->at(q).c_str());
            }
        } else if (!_isReachBound ){
            for(q = 0; q<numberOfQueries; q++){
                if (solved[q] != 1)
            ...
            for(q = 0; q<numberOfQueries; q++){
                for (int i = 0; i < _parser->queries[q].placebounds.size(); ++i)
                {
                    fprintf(successor_generator, "if(%s > placebound%d[%d]){ placebound%d[%d] = %s; }\n",  _parser->queries[q].placebounds[i].c_str(), q, i, q, i, _parser->queries[q].placebounds[i].c_str());
                }
            }
        }
        else {
...
// -- new
                ...
                fprintf(successor_generator, "MAX[%d", q);
                fputs("]); \n", successor_generator);

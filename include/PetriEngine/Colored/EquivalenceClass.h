...
class EquivalenceClass {
  public:
    ...
    ~EquivalenceClass() {}
    std::string to_string() const { return _colorIntervals.to_string(); }
    ...
    bool contains_color(const std::vector<uint32_t> &ids,
    ...
    EquivalenceClass subtract(uint32_t id, const EquivalenceClass &other,
                              const std::vector<bool> &diagonalPositions) const;

...
struct Retval {
    ...
    Retval(const std::shared_ptr<PQL::Condition> formula, AbstractProgramCollection_ptr &&lps1,
    ...
    Retval(const std::shared_ptr<PQL::Condition> formula, const AbstractProgramCollection_ptr &lps1,
    ...
    Retval(const std::shared_ptr<PQL::Condition> formula) : _formula(formula) {
    ...
    Retval(const Retval &&other)
        : _formula(std::move(other._formula)), _lps(std::move(other._lps)),

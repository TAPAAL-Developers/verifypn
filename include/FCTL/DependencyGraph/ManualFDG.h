...
#ifndef VERIFYPN_MANUALFDG_H
#define VERIFYPN_MANUALFDG_H
...
namespace Featured {
    namespace DependencyGraph {
        ...
        class ManualFDG : public BasicDependencyGraph {
        public:
            ManualFDG() : dict_(spot::make_bdd_dict()) {}
            ...
            void set_assignment(const IDType& id, bdd good, bdd bad) {
                auto config = get_config(id);
                config->good = good;
                config->bad = bad;
            }

            void add_hyperedge(const IDType& id, const std::vector<IDType>& sucs, const std::string &feat = "") {
                ...
                bdd formula = bddtrue;
                if (!feat.empty()) {
                    auto parsed = spot::parse_infix_psl(feat);
            ...
            void reset_state() {
                for (auto& [c, es]: edges_) {
                    for (auto* e: es) {
                        ...
                        e->processed = false;
                        e->refcnt = 1;
                        e->status = 0;//EdgeStatus::NotWaiting;
                    }
                }
                for (auto& [_, c]: configs_) {
                    c->nsuccs = 0;
         ...
         */
        template<typename T>
        void parse_assign_(const std::string& s, ManualFDG<T>& graph) {
        ...
        template<typename T>
        void parse_negation_(const std::string& s, ManualFDG<T>& graph) {
            std::istringstream is{s};
            T source, target;
            is.get(); // skip '!'
            //is >> source;
        ...
        template<typename T>
        void parse_hyperedge_(const std::string& s, ManualFDG<T>& graph) {
            std::istringstream is{s};
            T id;
            is >> id >> std::ws;
            T suc;
         ...
         */
        template<typename T>
        ManualFDG<T> parse_dg(std::istream& is) {
            ManualFDG<T> dg;
        ...
        template<typename T>
        ManualFDG<T> parse_dg(const std::string& s) {
            std::stringstream ss{s};
            return parse_dg<T>(ss);
        }

        template<typename T>
        ManualFDG<T> parse_dg(const char* s) {
            std::stringstream ss{s};
            return parse_dg<T>(ss);
        }
    }
}
#endif //VERIFYPN_MANUALFDG_H

 ...
 */
template <template <typename, typename...> typename ProductSucGen, typename SuccessorGen,
          bool SaveTrace = false, typename... Spooler>
class TarjanModelChecker : public ModelChecker<ProductSucGen, SuccessorGen, Spooler...> {
  public:
    TarjanModelChecker(const PetriEngine::PetriNet &net,
        ...
        : ModelChecker<ProductSucGen, SuccessorGen, Spooler...>(net, cond, buchi, successorGen,
                                                                std::move(spooler)...),
          _seen(net, 0) {
        if (buchi._buchi->num_states() > 65535) {
            throw base_error_t(
                "Fatal error: cannot handle Büchi automata larger than 2^16 states\n");
        }
    ...
    void print_stats(std::ostream &os) override {
        ModelChecker<ProductSucGen, SuccessorGen, Spooler...>::print_stats(os, _seen);
    }
...
extern template class TarjanModelChecker<ProductSuccessorGenerator, LTL::SpoolingSuccessorGenerator,
                                         true>;

extern template class TarjanModelChecker<ProductSuccessorGenerator, LTL::SpoolingSuccessorGenerator,
                                         false>;

extern template class TarjanModelChecker<ReachStubProductSuccessorGenerator,
                                         LTL::SpoolingSuccessorGenerator, true,
                                         VisibleLTLStubbornSet>;

extern template class TarjanModelChecker<ReachStubProductSuccessorGenerator,
                                         LTL::SpoolingSuccessorGenerator, false,
                                         VisibleLTLStubbornSet>;

...
class SpoolingSuccessorGenerator : public PetriEngine::SuccessorGenerator {
  public:
    SpoolingSuccessorGenerator(const PetriEngine::PetriNet &net,
                               const PetriEngine::PQL::Condition_ptr &)
        : SuccessorGenerator(net), _transbuf(new uint32_t[net.number_of_transitions()]) {
        _statebuf.set_marking(new PetriEngine::MarkVal[net.number_of_places() + 1],
                              net.number_of_places());
    }
    ...
    void prepare(const Structures::ProductState &state, successor_info_t &sucinfo) {
        ...
        if (sucinfo._successors == nullptr) {
            ...
            if (!_heuristic || !_heuristic->has_heuristic(state)) {
                ...
                sucinfo._successors =
                    SuccessorQueue(weighted_tids, [](auto &p) { return p.first; });
            }
        }
    }
...
#ifndef NDEBUG
            // std::cerr << "Not Firing: " << (sucinfo.successors.has_consumed() ? "deadlock" :
            // "done") << std::endl;
#endif
    ...
    void generate_all(LTL::Structures::ProductState &parent, successor_info_t &sucinfo) {
        assert(_spooler != nullptr);
        assert(sucinfo._successors != nullptr);
        if (!_spooler->generate_all(parent))
            return;
        ...
        if (!_heuristic) {
            ...
            std::transform(std::begin(weighted_tids), std::end(weighted_tids), _transbuf.get(),
    ...
    void push() {
        ...
        if (!_heuristic || fired() == std::numeric_limits<uint32_t>::max())
            return;
  ...
  private:
    ...
    std::unique_ptr<uint32_t[]>
        _transbuf; /* buffer for enabled transitions, size is ntransitions. */

...
class ReachStubProductSuccessorGenerator : public ProductSuccessorGenerator<S> {
  public:
    ReachStubProductSuccessorGenerator(const PetriEngine::PetriNet &net,
                                       const Structures::BuchiAutomaton &buchi, S *successorGen,
                                       std::unique_ptr<Spooler> &&fallbackSpooler)
        : ProductSuccessorGenerator<S>(net, buchi, successorGen),
          _fallback_spooler(std::move(fallbackSpooler)) {
        ...
        /*            if constexpr (std::is_same_v<Spooler, EnabledSpooler>) {
           ...
           static_cast<PetriEngine::SuccessorGenerator&>(this->_successor_generator));
                    }
                    else {
                        _fallback_spooler = std::make_unique<Spooler>(net);
                    }*/
    ...
    void prepare(const LTL::Structures::ProductState &state,
                 typename S::successor_info_t &sucinfo) override {
        ...
        if (suc != std::end(_reach_states) && !this->guard_valid(state, suc->second._bddCond)) {
            ...
            _reach->set_buchi_conds(suc->second._ret_cond, suc->second._prog_cond,
                                    suc->second._pseudo_sink_cond);

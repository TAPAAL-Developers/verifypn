...
class WeightedComposedHeuristic : public ComposedHeuristic {
  public:
    WeightedComposedHeuristic(std::unique_ptr<Heuristic> primary,
                              std::unique_ptr<Heuristic> secondary, uint32_t primary_weight,
                              uint32_t secondary_weight)
        : ComposedHeuristic(std::move(primary), std::move(secondary)),
          _primary_weight(primary_weight), _secondary_weight(secondary_weight) {}
    ...
    uint32_t eval(const Structures::ProductState &state, uint32_t tid) override {
        if (_primary_has_heuristic && !_secondary_has_heuristic) {
            ...
            return _primary->eval(state, tid) * _primary_weight +
                   _secondary->eval(state, tid) * _secondary_weight;
        } else {
            return 0;
        }
    }

  ...
  public:
    ...
    auto next(LTL::Structures::ProductState &state) -> bool {
        if (_fresh_marking) {
            ...
            if (!_successor_generator->next(state)) {
                // This is a fresh marking, so if there is no more successors for the state the
                // state is deadlocked. The semantics for deadlock is to just loop the marking so
                // return true without changing the value of state.
                std::copy(_successor_generator->parent(),
                          _successor_generator->get_parent() + state._buchi_state_idx + 1,
                          state.marking());
            }
        }
    ...
    auto make_initial_state() -> std::vector<LTL::Structures::ProductState> {
        ...
        while (next_buchi_succ(state)) {
            ...
            states.back().set_marking(new PetriEngine::MarkVal[_net.number_of_places() + 1],
                                      _net.number_of_places());
            std::copy(state.marking(), state.marking() + _net.number_of_places(),
                      states.back().marking());
    ...
    virtual void prepare(const LTL::Structures::ProductState &state,
        ...
        if (!_fresh_marking) {
            ...
            // Caveat: it seems like there usually are not that many successors, so this is probably
            // cheap regardless
            size_t tmp;
    ...
    auto next(Structures::ProductState &state, typename SuccessorGen::successor_info_t &sucinfo)
        ...
        if (_fresh_marking) {
            ...
            if (!_successor_generator->next(state, sucinfo)) {
                // This is a fresh marking, so if there are no more successors for the state the
                // state is deadlocked. The semantics for deadlock is to just loop the marking so
                // return true without changing the value of state.
                std::copy(_successor_generator->get_parent(),
                          _successor_generator->get_parent() + state._buchi_state_idx + 1,
                          state.marking());
            }
        }

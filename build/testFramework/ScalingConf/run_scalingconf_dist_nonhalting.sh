...
do
    ...
    do
        ...
        do
            let timeout="$max_timeout"
            sbatch /user/smni12/launchpad/master/build/testFramework/ScalingConf/compute6.sh "$model" "$worker_count" "$timeout" "$i" "$RUN_NO" "$BINARY"
        done
    done
done
...
do
    ...
    do
        ...
        do
            let timeout="$max_timeout"
            sbatch /user/smni12/launchpad/master/build/testFramework/ScalingConf/compute8.sh "$model" "$worker_count" "$timeout" "$i" "$RUN_NO" "$BINARY"
        done
    done
done

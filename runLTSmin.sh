...
if [[ $LTSMINTYPE == "-mc" ]]
then
    ${LTSMINPATH}pins2lts-mc --strategy=dfs --procs=8 --state=table --invariant="! goal" $PREFIX/autogenerated/sharedfile.so 
else
    ${LTSMINPATH}pins2lts-seq --strategy=dfs --state=table --invariant="! goal" $PREFIX/autogenerated/sharedfile.so 
fi

...
PREFIX=/home/mcc/BenchKit/bin/onthefly/verifypnLTSmin
...
then
    ...
    if [[ $ISDEADLOCK != "true" ]] 
    then
    	${LTSMINPATH}pins2lts-mc --strategy=dfs --procs=4 --state=table --invariant="! goal" --when $PREFIX/autogenerated/sharedfile.so
    else
	${LTSMINPATH}pins2lts-mc --strategy=dfs --procs=4 --state=table --deadlock --when $PREFIX/autogenerated/sharedfile.so 
    fi

else
    if [[ $ISDEADLOCK != "true" ]] 
    then
        ${LTSMINPATH}pins2lts-seq --strategy=dfs --state=table --invariant="! goal" --when $PREFIX/autogenerated/sharedfile.so 
    else

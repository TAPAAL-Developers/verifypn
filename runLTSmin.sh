...
LTSMINTYPE=$1
ISDEADLOCK=$2
PREFIX=/home/mads/verifypnLTSmin/verifypnLTSmin
...
then
    if [[$ISDEADLOCK != "true"]] 
    then
    	${LTSMINPATH}pins2lts-mc --strategy=dfs --procs=8 --state=table --invariant="! goal" --when $PREFIX/autogenerated/sharedfile.so
    else
	${LTSMINPATH}pins2lts-mc --strategy=dfs --procs=8 --state=table --deadlock --when $PREFIX/autogenerated/sharedfile.so 
    fi
else
    if [[$ISDEADLOCK != "true"]] 
    then

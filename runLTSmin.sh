...
then
    ...
    if [[ $ISDEADLOCK != "true" ]] 
    then
    	${LTSMINPATH}pins2lts-mc --strategy=$STRAT --procs=$NRCORES --state=table --invariant="! goal" --when $PREFIX/autogenerated/sharedfile.so
    else
    ...
    if [[ $ISDEADLOCK != "true" ]] 
    then
        ${LTSMINPATH}pins2lts-seq --strategy=$STRAT --state=table --invariant="! goal" --when $PREFIX/autogenerated/sharedfile.so 
    else
	${LTSMINPATH}pins2lts-seq --strategy=$STRAT --state=table --deadlock --when $PREFIX/autogenerated/sharedfile.so
    fi
fi

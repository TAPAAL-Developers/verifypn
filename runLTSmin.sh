...
if [[ $LTSMINTYPE == "-mc" ]]
then
    ${LTSMINPATH}pins2lts-mc --procs=8 --state=table --invariant="! goal" --trace=solution.gcf --when $PREFIX/autogenerated/sharedfile.so 
else
    ${LTSMINPATH}pins2lts-seq --state=table --invariant="! goal" --trace=solution.gcf --when $PREFIX/autogenerated/sharedfile.so 
fi

...
then
	if [[$ISDEADLOCK != "true"]] 
    then
    	${LTSMINPATH}pins2lts-mc --procs=8 --state=table --invariant="! goal" --when $PREFIX/autogenerated/sharedfile.so
	else
		${LTSMINPATH}pins2lts-mc --procs=8 --state=table --deadlock --when $PREFIX/autogenerated/sharedfile.so 
    fi
else
    if [[$ISDEADLOCK != "true"]] 
    then
		...
		${LTSMINPATH}pins2lts-seq --state=table --deadlock --when $PREFIX/autogenerated/sharedfile.so
    fi
fi
